...
public class OutboxInsertBenchmarks
{
    ...
    public async Task SetupIteration()
    {
        ...
        await using var seed = new NpgsqlCommand(
            $"""
             ...
             SELECT 'pool-' || g, @payload, 12, 5, 0, 0, 0, 0
             FROM generate_series(1, 5000) AS g;
             """,
            _insertConnection);
        seed.Parameters.AddWithValue("payload", "{}");
        await seed.ExecuteNonQueryAsync();
    }

    /// <summary>UNNEST 批量 INSERT（payload_utf8 预序列化，payload_json 写 NULL）。</summary>
    [Benchmark(Description = "Outbox INSERT (UNNEST, payload_utf8 only)")]
    public async Task InsertOutboxMany()
    {
    ...
    /// <summary>Claim 一批 10 条 Pending 行（FOR UPDATE SKIP LOCKED，0 JSON 反序列化）。</summary>
    [Benchmark(Description = "Outbox Claim (0 JSON parse)")]
    public async Task ClaimBatch()
    {
