...
public sealed class NatsMessageRecallConsumer : IMessageRecallConsumer
{
    ...
    private readonly ILogger<NatsMessageRecallConsumer> _logger;

    public NatsMessageRecallConsumer(
        RealtimeQueueOptions options,
        RealtimeNatsTrustSettings trust,
        NatsConnectionClient connectionClient,
        ILogger<NatsMessageRecallConsumer> logger)
    {
        _options = options;
        _trust = trust;
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        _logger.LogInformation(
            "NATS 消息撤回端点已订阅。Subject={Subject}；队列组={QueueGroup}",
            _options.Topics.MessageRecalls,
        ...
        {
            MessageRecallCommand? command = null;
            try
            {
                msg.EnsureSuccess();
                if (!string.IsNullOrWhiteSpace(msg.Data))
                {
                    command = JsonSerializer.Deserialize(
                        msg.Data,
                        RealtimeJsonSerializerContext.Default.MessageRecallCommand);
                }
            }
            catch (JsonException ex)
            {
                _logger.LogWarning(
                    ex,
                    "NATS 消息撤回命令反序列化失败。Subject={Subject}",
                    msg.Subject);
            }

            if (command is null)
            {
                var invalid = MessageRecallResult.Failed(
                    string.Empty,
                    "invalid_command_json",
                    "消息撤回命令负载为空或格式无效。");
                var invalidJson = JsonSerializer.Serialize(
                    invalid,
                    RealtimeJsonSerializerContext.Default.MessageRecallResult);
            ...
            yield return new MessageRecallEnvelope(
                command,
