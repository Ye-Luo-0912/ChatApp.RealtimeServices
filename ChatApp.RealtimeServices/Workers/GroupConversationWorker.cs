...
public sealed class GroupConversationWorker : BackgroundService
{
    ...
    private readonly ILogger<GroupConversationWorker> _logger;

    public GroupConversationWorker(
        IGroupConversationConsumer consumer,
        IGroupConversationProcessor processor,
        RealtimeReadinessState readinessState,
        RealtimeMetrics metrics,
        IOptions<RealtimeOptions> options,
        RealtimeQueryConcurrencyGate queryGate,
        RealtimeNatsTrustSettings trust,
        ILogger<GroupConversationWorker> logger)
    {
        _consumer = consumer;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var channel = Channel.CreateBounded<GroupConversationEnvelope>(
    ...
    private async Task ProduceAsync(
        ChannelWriter<GroupConversationEnvelope> writer,
            ...
            try
            {
                                   ...
                                   .ConfigureAwait(false))
                {
                    ...
                    if (_options.OverloadEnqueueTimeoutMs > 0)
                    {
                        ...
                        {
                            _metrics.RecordOverloadReply("group_conversation", "enqueue");
                            await envelope.ReplyAsync(
                                GroupConversationResult.ServerBusy(
    ...
    private async Task ProcessAsync(
        int workerIndex,
        ChannelReader<GroupConversationEnvelope> reader,
        CancellationToken ct)
    {
        await foreach (var envelope in reader.ReadAllAsync(ct).ConfigureAwait(false))
        {
            ...
            try
            {
                ...
                if (!gateAcquired)
                {
                    _metrics.RecordOverloadReply("group_conversation", "gate");
                    outcome = "server_busy";
                    try
                    {
                        await envelope.ReplyAsync(
