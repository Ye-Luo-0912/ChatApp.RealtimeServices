...
public sealed class ConversationSyncBootstrapWorker : BackgroundService
{
    ...
    private readonly ILogger<ConversationSyncBootstrapWorker> _logger;

    public ConversationSyncBootstrapWorker(
        ISyncBootstrapQueryConsumer consumer,
        ISyncBootstrapQueryProcessor processor,
        RealtimeReadinessState readinessState,
        RealtimeMetrics metrics,
        IOptions<RealtimeOptions> options,
        RealtimeQueryConcurrencyGate queryGate,
        RealtimeNatsTrustSettings trust,
        ILogger<ConversationSyncBootstrapWorker> logger)
    {
        _consumer = consumer;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var channel = Channel.CreateBounded<SyncBootstrapQueryEnvelope>(
        ...
        {
            _logger.LogInformation("会话同步引导工作器正在停止。");
        }
        catch (Exception ex)
        {
            _readinessState.MarkFaulted(WorkerName, ex);
            _logger.LogError(ex, "会话同步引导订阅循环异常退出。");
            throw;
        }
        finally
        {
    ...
    private async Task ProduceAsync(
        ChannelWriter<SyncBootstrapQueryEnvelope> writer,
            ...
            try
            {
                                   ...
                                   .ConfigureAwait(false))
                {
                    ...
                    if (_options.OverloadEnqueueTimeoutMs > 0)
                    {
                        ...
                        {
                            _metrics.RecordOverloadReply("sync_bootstrap", "enqueue");
                            await envelope.ReplyAsync(
                                SyncBootstrapPage.ServerBusy(
    ...
    private async Task ProcessAsync(
        int workerIndex,
        ChannelReader<SyncBootstrapQueryEnvelope> reader,
        CancellationToken ct)
    {
        await foreach (var envelope in reader.ReadAllAsync(ct).ConfigureAwait(false))
        {
            ...
            try
            {
                ...
                if (!gateAcquired)
                {
                    _metrics.RecordOverloadReply("sync_bootstrap", "gate");
                    outcome = "server_busy";
                    try
                    {
                        await envelope.ReplyAsync(
