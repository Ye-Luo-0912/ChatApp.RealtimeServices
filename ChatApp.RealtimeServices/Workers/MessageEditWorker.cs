...
public sealed class MessageEditWorker : BackgroundService
{
    ...
    private readonly ILogger<MessageEditWorker> _logger;

    public MessageEditWorker(
        IMessageEditConsumer consumer,
        IMessageEditProcessor processor,
        RealtimeReadinessState readinessState,
        RealtimeMetrics metrics,
        IOptions<RealtimeOptions> options,
        RealtimeQueryConcurrencyGate queryGate,
        RealtimeNatsTrustSettings trust,
        ILogger<MessageEditWorker> logger)
    {
        _consumer = consumer;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var channel = Channel.CreateBounded<MessageEditEnvelope>(
        ...
        {
            _logger.LogInformation("消息编辑工作器正在停止。");
        }
        catch (Exception ex)
        {
            _readinessState.MarkFaulted(WorkerName, ex);
            _logger.LogError(ex, "消息编辑订阅循环异常退出。");
            throw;
        }
        finally
        {
    ...
    private async Task ProduceAsync(
        ChannelWriter<MessageEditEnvelope> writer,
    ...
    private async Task ProcessAsync(
        int workerIndex,
        ChannelReader<MessageEditEnvelope> reader,
        CancellationToken ct)
    {
