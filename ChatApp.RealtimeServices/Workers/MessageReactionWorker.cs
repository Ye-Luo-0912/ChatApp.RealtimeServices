...
public sealed class MessageReactionWorker : BackgroundService
{
    private const string WorkerName = nameof(MessageReactionWorker);
    private readonly IMessageReactionConsumer _consumer;
    private readonly IMessageReactionProcessor _processor;
    ...
    private readonly ILogger<MessageReactionWorker> _logger;

    public MessageReactionWorker(
        IMessageReactionConsumer consumer,
        IMessageReactionProcessor processor,
        RealtimeReadinessState readinessState,
        RealtimeMetrics metrics,
        IOptions<RealtimeOptions> options,
        RealtimeQueryConcurrencyGate queryGate,
        RealtimeNatsTrustSettings trust,
        ILogger<MessageReactionWorker> logger)
    {
        _consumer = consumer;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        _logger.LogInformation(
            "消息反应工作器已启动。共享并发={Concurrency}；队列容量={Capacity}；工作槽={Slots}",
            _queryGate.Permits,
        ...
        var channel = Channel.CreateBounded<MessageReactionEnvelope>(
        ...
        {
            _logger.LogInformation("消息反应工作器正在停止。");
        }
        catch (Exception ex)
        {
            _readinessState.MarkFaulted(WorkerName, ex);
            _logger.LogError(ex, "消息反应订阅循环异常退出。");
            throw;
        }
        finally
        {
    ...
    private async Task ProduceAsync(
        ChannelWriter<MessageReactionEnvelope> writer,
    ...
    private async Task ProcessAsync(
        int workerIndex,
        ChannelReader<MessageReactionEnvelope> reader,
        CancellationToken ct)
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            result = MessageReactionResult.Failed(
                                envelope.Command.RequestId,
                                identityError,
                                "网关身份校验失败：反应操作方与可信身份头不匹配或缺失。");
                        }
                        else
                        {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _logger.LogError(
                            ex,
                            "消息反应失败。工作槽={WorkerIndex}；请求编号={RequestId}",
                            workerIndex,
                            envelope.Command.RequestId);
                        result = MessageReactionResult.Failed(
                            envelope.Command.RequestId,
                            "message_reaction_unavailable",
