...
public sealed class ConversationSetPrefsWorker : BackgroundService
{
    private const string WorkerName = nameof(ConversationSetPrefsWorker);
    private readonly IConversationSetPrefsConsumer _consumer;
    private readonly IConversationSetPrefsProcessor _processor;
    ...
    private readonly ILogger<ConversationSetPrefsWorker> _logger;

    public ConversationSetPrefsWorker(
        IConversationSetPrefsConsumer consumer,
        IConversationSetPrefsProcessor processor,
        RealtimeReadinessState readinessState,
        RealtimeMetrics metrics,
        IOptions<RealtimeOptions> options,
        RealtimeQueryConcurrencyGate queryGate,
        RealtimeNatsTrustSettings trust,
        ILogger<ConversationSetPrefsWorker> logger)
    {
        _consumer = consumer;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        _logger.LogInformation(
            "会话偏好设置工作器已启动。共享并发={Concurrency}；队列容量={Capacity}；工作槽={Slots}",
            _queryGate.Permits,
        ...
        var channel = Channel.CreateBounded<ConversationSetPrefsEnvelope>(
        ...
        {
            _logger.LogInformation("会话偏好设置工作器正在停止。");
        }
        catch (Exception ex)
        {
            _readinessState.MarkFaulted(WorkerName, ex);
            _logger.LogError(ex, "会话偏好设置订阅循环异常退出。");
            throw;
        }
        finally
        {
    ...
    private async Task ProduceAsync(
        ChannelWriter<ConversationSetPrefsEnvelope> writer,
            ...
            try
            {
                                   ...
                                   .ConfigureAwait(false))
                {
                    ...
                    if (_options.OverloadEnqueueTimeoutMs > 0)
                    {
                        ...
                        {
                            _metrics.RecordOverloadReply("conversation_set_prefs", "enqueue");
                            await envelope.ReplyAsync(
                                ConversationSetPrefsResult.ServerBusy(
    ...
    private async Task ProcessAsync(
        int workerIndex,
        ChannelReader<ConversationSetPrefsEnvelope> reader,
        CancellationToken ct)
    {
        await foreach (var envelope in reader.ReadAllAsync(ct).ConfigureAwait(false))
        {
            ...
            using var activity = RealtimeTelemetry.StartConsumer(
                "conversation_set_prefs.process",
            ...
            try
            {
                ...
                if (!gateAcquired)
                {
                    _metrics.RecordOverloadReply("conversation_set_prefs", "gate");
                    outcome = "server_busy";
                    try
                    {
                        await envelope.ReplyAsync(
                ...
                try
                {
                    ConversationSetPrefsResult result;
                    try
                    {
                        ...
                        {
                            result = ConversationSetPrefsResult.Failed(
                                envelope.Command.RequestId,
                                identityError,
                                "网关身份校验失败：会话偏好用户与可信身份头不匹配或缺失。");
                        }
                        else
                        {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _logger.LogError(
                            ex,
                            "会话偏好设置失败。工作槽={WorkerIndex}；请求编号={RequestId}",
                            workerIndex,
                            envelope.Command.RequestId);
                        result = ConversationSetPrefsResult.Failed(
                            envelope.Command.RequestId,
                            "conversation_set_prefs_unavailable",
