            ...
            try
            {
                                   ...
                                   .ConfigureAwait(false))
                {
                    ...
                    if (_options.OverloadEnqueueTimeoutMs > 0)
                    {
                        ...
                        {
                            _metrics.RecordOverloadReply("message_recall", "enqueue");
                            await envelope.ReplyAsync(
                                MessageRecallResult.ServerBusy(
        ...
        CancellationToken ct)
    {
        await foreach (var envelope in reader.ReadAllAsync(ct).ConfigureAwait(false))
        {
            ...
            try
            {
                ...
                if (!gateAcquired)
                {
                    _metrics.RecordOverloadReply("message_recall", "gate");
                    outcome = "server_busy";
                    try
                    {
                        await envelope.ReplyAsync(
