...
public static class PostgresPerfReporter
{
        ...
        string phaseTitle)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("| queryid | calls | exec(ms) | rows | blks_read | dirtied | wal_records | wal_bytes | sql |");
        sb.AppendLine("|---|---|---|---|---|---|---|---|---|");
        foreach (var s in diff.Statements.Take(30))
        {
            var sql = s.Query.Length > 90 ? s.Query[..90] + "…" : s.Query;
            sb.AppendLine($"| {s.QueryId} | {s.Calls:N0} | {s.TotalExecMs:N1} | {s.Rows:N0} | {s.SharedBlksRead:N0} | {s.SharedBlksDirtied:N0} | {s.WalRecords:N0} | {s.WalBytes:N0} | `{Sanitize(sql)}` |");
        }
        ...
        sb.AppendLine();
        sb.AppendLine("| queryid | calls | wal_bytes | wal_records | sql |");
        sb.AppendLine("|---|---|---|---|---|");
        foreach (var s in diff.Statements.OrderByDescending(s => s.WalBytes).Take(15))
        {
            var sql = s.Query.Length > 90 ? s.Query[..90] + "…" : s.Query;
            sb.AppendLine($"| {s.QueryId} | {s.Calls:N0} | {s.WalBytes:N0} | {s.WalRecords:N0} | `{Sanitize(sql)}` |");
        }
