...
- 需求 2 单项验证（减少重复读取·授权读取成本）：A/B 量化合并路径 admission 段「同会话重复授权读取」的每消息成本。`BuildSuperBundleCommandText` 抽出 `includeAuthorization` 开关生成 no-auth 变体（`write_gate` 只按生命周期 + 幂等 canonical 门控，admission 读 5 张表 → 2 张表），`RunNoAuthSuperBundleWorkloadAsync` 驱动，`SaveAsync_NoAuthReadsAb_ReportsAuthorizationReadCost` 测量测试。A/B 语句数与写入行集完全相同（单条 CTE、conversations/members/messages/outbox/ledger 各 1 行），唯一差异是 4 处授权表读取（AspNetUsers 2 行 + T_BlockRecords 1 次 EXISTS + T_UserFriendEntry 2 次 EXISTS）。结果：热路径 SQL/消息与 SQL 往返/消息不变（1 / 8.0，确认 B 未改变语句结构）；总执行时间/消息 0.67 → 0.59 ms（约 −12%，即授权读取每消息约 0.08 ms）；WAL 列受短窗口容器波动影响不可靠归因（语句级 B 甚至略高），不采信。B 为测量专用变体（跳过授权在语义上不安全），授权读取成本非可忽略，后续可评估「会话级授权缓存 / 已建会话免重复授权」方向。报告存档 `docs/measurements/outbox-db-auth-reads-ab.md`。
- 需求 2 单项验证（减少重复读取·会话级授权缓存）：A/B 量化「已建会话免重复授权」的每消息收益。`PostgresPerfHarness` 扩展多会话语料分布（`ConversationSlot`：count 消息按 msgsPerConv=count/conversationCount 分到 conversationCount 个会话，接收者 = 10_000_000_002+convIndex），`SeedAuthTablesAsync` 支持 receiverCount 播种各会话接收者授权关系；新增 `RunSessionAuthCacheWorkloadAsync`（每会话首条完整授权读取建立会话、后续 49 条跳过授权读取），`SaveAsync_SessionAuthCacheAb_ReportsEstablishedSessionSaving` 测量测试。A 每条消息完整授权读取、B 会话级授权缓存，语句数与写入行集完全相同、会话分布完全相同，唯一差异是授权读取粒度。结果：热路径 SQL/消息与 SQL 往返/消息不变（1 / 8.0，确认 B 未改变语句结构）；总执行时间/消息 0.54 → 0.40 ms（约 −26%）；WAL 字节/消息 2,976 vs 3,028 受短窗口容器波动不可靠归因，不采信。理论收益上限为回收 (msgsPerConv−1)/msgsPerConv ≈ 98% 的授权读取成本（单次约 0.08 ms）。会话级授权缓存方向可行，落地生产需对「会话建立」与「授权事实变更」做 TTL 或显式失效注入，使缓存命中不绕过会话建立后的授权变化；B 为测量专用变体，不直接落生产。报告存档 `docs/measurements/outbox-db-session-auth-cache-ab.md`。
- 需求 2 单项验证（减少重复读取·发送者状态冗余读取）：A/B 量化「仅读接收者 user_state」的每消息收益（发送者已认证免重复读取）。`BuildSuperBundleCommandTextCore` 增加 `senderKnown` 开关（`direct_user_state` 只读取接收者行 `WHERE "Id" = $3`、`sender_exists` 固定为 TRUE），新增 `RunSenderKnownSuperBundleWorkloadAsync` 与 `SaveAsync_SenderKnownAb_ReportsSenderStateReadSaving` 测量测试。A 每条消息读发送者+接收者两行、B 只读接收者行，语句数与写入行集完全相同、授权判定与写入路径不变。结果：总执行时间/消息差仅 0.01–0.02 ms（0.31–0.35 → 0.30–0.34 ms），处于测量噪声内（多轮运行 WAL 差从 −10% 到 −0.6% 大幅漂移）；语句级 main CTE exec/消息 两者持平、blks_read 均为 0（direct_user_state 两行均命中共享缓冲区）。结论为负收益：sender 行重读不是热路径热点，不值得为消除 sender 行读取引入生产改动（会破坏 direct_user_state 作为 admission 事实源的一致语义却无性能回报）；后续减少重复读取应聚焦授权判定（direct_authorization）而非 direct_user_state 的发送者存在性行。报告存档 `docs/measurements/outbox-db-sender-known-ab.md`。
- 需求 2 单项验证（避免无变化 UPDATE·会话头列守卫）：A/B 量化 upsert_conversation 的 CASE 守卫（保留未推进的会话头列）相对无条件覆盖的 HOT/索引维护收益。`BuildSuperBundleCommandTextCore` 增加 `guardConversationHeader` 开关生成 naive 变体（无条件覆盖 last_message_id/preview/at_ms/sender_user_id），`PostgresPerfHarness` 新增乱序语料（第 0 条极高 received_at_ms 建立会话头高水位、后续消息均低于高水位使会话头不再推进）与 `RunOutOfOrderGuardedWorkloadAsync`/`RunOutOfOrderNaiveWorkloadAsync`，`SaveAsync_NoopUpdateAb_ReportsConversationHeaderSaving` 测量测试。结果：A 与 B 的 conversations 表 HOT 命中率均为 100%（2,000/2,000），无差异。**结论性发现：该子方向已被 Migration058 吸收**——`Migration058_ConversationHotProjectionUpdates` 已 DROP 含 `last_message_at_ms` 的全局索引 `ix_conversations_last_message_list`（其不覆盖 user/pinned 谓词、正式 8 小时运行扫描次数为 0，却让 230 万次 tip 更新全部无法 HOT），索引移除后该列已非索引列，无论是否用 CASE 守卫会话 tip 更新都走 HOT，不再触发索引维护。CASE 守卫可继续保留（逻辑上避免无意义覆写、减少 dead tuple 与 WAL 写放大），但不存在进一步的 HOT/索引类收益可压测，该列为「已被既有迁移吸收、无需再优化」的负向结论。报告存档 `docs/measurements/outbox-db-noop-update-ab.md`。
- 需求 2 单项验证（排水 HOT 更细归因）：把排水 aggregate HOT（fillfactor=50 下 87%）归因到 claim 语句，并隔离 delete 对 claim HOT 的页内空间争夺。`PostgresPerfHarness` 新增 `RunOutboxClaimOnlyAsync`（只 claim 不 delete），`DrainClaimHot_Isolation_AttributesPageSpaceContention` 测量测试 A/B 对比 claim-only vs claim+delete（两窗口前均 TRUNCATE、fillfactor=50）。**结论性发现：A 与 B 的 claim hot_updates 逐条相同（1,723/2,000 = 86%），而 outbox dead tuple 差异巨大（450 vs 2,286）——delete 生成的 dead tuple 对 claim HOT 无影响，原「delete 挤压页内版本链空间」假设被证伪**。residual 14% non-HOT 是 claim 语句自身的固有属性：INSERT 以 fillfactor=50 写入后页内已占用一半空间，单行在加入 lease 列后若体积超过该页剩余可分配空间即无法 HOT（跨页重定位 + 主键索引更新），该比例由「页填充率 vs 行更新后体积」物理布局决定、与是否删除无关。fillfactor=50 已把 aggregate HOT 顶到 86% 且不因删除路径退化；进一步逼近 100% 只能降 fillfactor 或压缩 payload，收益有限（仅影响 claim WAL 记录数，不影响已实现的 371 WAL 字节/消息）。报告存档 `docs/measurements/outbox-db-claim-hot-attribution-ab.md`。
