...
> 生成时间：2026-08-19 06:41:14 UTC；语料固定、随机种子 20260813；delete-on-complete 排水 2000 条。

| 批量上限 | 排水 SQL 往返/消息 | 排水耗时/消息(ms) | 排水 WAL 字节/消息 | claim 调用 | delete 调用 |
|---|---|---|---|---|---|
| A：1（每条消息独立 claim+delete） | 2.00 | 0.37 | 336 | 2000 | 2000 |
...
|---|---|---|---|---|---|
| 2,000 | 1.00 | 0.345 | 246 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "per…` |
| 2,000 | 1.00 | 0.024 | 89 | `DELETE FROM "perf_bc3e3f9d21e3"."outbox" AS item USING UNNEST($1, $2) AS…` |
...
|---|---|---|---|---|---|---|---|---|---|
| C5FA397E315F9D09 | 2,000 | 690.4 | 2,000 | 0 | 0 | 4,285 | 0 | 492,815 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_bc3e3f9d21e3"."o…` |
| E11B52EF572B8535 | 1 | 201.3 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| 26CAC6B03022E745 | 2,000 | 47.4 | 2,000 | 0 | 0 | 3,142 | 0 | 178,808 | `DELETE FROM "perf_bc3e3f9d21e3"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| 403F8B358778114D | 4,003 | 16.8 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 878A9E463E585A2C | 4,003 | 9.0 | 4,003 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 4040643821AD66AF | 4,003 | 8.5 | 0 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| D28E47E4803A0167 | 4,003 | 1.5 | 0 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| 92049CC8AB443DC6 | 4,003 | 1.2 | 0 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
| C0D0E27048376284 | 4,003 | 1.1 | 0 | 0 | 0 | 0 | 0 | 0 | `UNLISTEN *` |
| B71A600DFCB91748 | 1 | 0.4 | 232 | 0 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 516E8F1759460B47 | 4,003 | 0.4 | 0 | 0 | 0 | 0 | 0 | 0 | `DISCARD TEMP` |
...
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 2,000 | 492,815 | 4,285 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_bc3e3f9d21e3"."o…` |
| 26CAC6B03022E745 | 2,000 | 178,808 | 3,142 | 0 | `DELETE FROM "perf_bc3e3f9d21e3"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
...
| 语句 | 调用 | 调用/消息 | exec/消息(ms) | wal_bytes/调用 | sql 片段 |
|---|---|---|---|---|---|
| 10 | 0.01 | 0.010 | 74,424 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "per…` |
| 10 | 0.01 | 0.003 | 13,144 | `DELETE FROM "perf_bc3e3f9d21e3"."outbox" AS item USING UNNEST($1, $2) AS…` |
...
|---|---|---|---|---|---|---|---|---|---|
| E11B52EF572B8535 | 1 | 201.3 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| C5FA397E315F9D09 | 10 | 20.8 | 2,000 | 0 | 48 | 5,424 | 0 | 744,240 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_bc3e3f9d21e3"."o…` |
| 26CAC6B03022E745 | 10 | 5.8 | 2,000 | 0 | 0 | 2,291 | 0 | 131,441 | `DELETE FROM "perf_bc3e3f9d21e3"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 10 | 744,240 | 5,424 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_bc3e3f9d21e3"."o…` |
| 26CAC6B03022E745 | 10 | 131,441 | 2,291 | 0 | `DELETE FROM "perf_bc3e3f9d21e3"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| E11B52EF572B8535 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| B71A600DFCB91748 | 1 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
