...
> 生成时间：2026-08-13 20:56:06 UTC；语料固定、随机种子 20260813；每配置 inbound 2000 条消息、delete-on-complete 排水 2000 条。
...
| C5FA397E315F9D09 | 40 | 38.5 | 2,000 | 0 | 357 | 5,319 | 0 | 719,936 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_068fcaba1efa"."o…` |
| 26CAC6B03022E745 | 40 | 21.1 | 2,000 | 0 | 0 | 2,305 | 0 | 132,165 | `DELETE FROM "perf_068fcaba1efa"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| 403F8B358778114D | 83 | 0.5 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| B71A600DFCB91748 | 1 | 0.5 | 233 | 0 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 4040643821AD66AF | 83 | 0.4 | 0 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
...
| C0D0E27048376284 | 83 | 0.1 | 0 | 0 | 0 | 0 | 0 | 0 | `UNLISTEN *` |
...
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 40 | 719,936 | 5,319 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_068fcaba1efa"."o…` |
| 26CAC6B03022E745 | 40 | 132,165 | 2,305 | 0 | `DELETE FROM "perf_068fcaba1efa"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
...
| C5FA397E315F9D09 | 40 | 62.0 | 2,000 | 0 | 44 | 5,314 | 0 | 719,461 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_068fcaba1efa"."o…` |
| 26CAC6B03022E745 | 40 | 37.5 | 2,000 | 0 | 0 | 2,304 | 0 | 132,106 | `DELETE FROM "perf_068fcaba1efa"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| B71A600DFCB91748 | 1 | 0.7 | 237 | 0 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 403F8B358778114D | 83 | 0.7 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 83 | 0.5 | 0 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| 878A9E463E585A2C | 83 | 0.3 | 83 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 40 | 719,461 | 5,314 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_068fcaba1efa"."o…` |
| 26CAC6B03022E745 | 40 | 132,106 | 2,304 | 0 | `DELETE FROM "perf_068fcaba1efa"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| E11B52EF572B8535 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| B71A600DFCB91748 | 1 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 403F8B358778114D | 83 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 83 | 0 | 0 | 0 | `RESET ALL` |
