...
> 生成时间：2026-08-13 18:52:53 UTC；语料固定、随机种子 20260813；每配置 inbound 2000 条消息，其中约 1/4 携带 reply_to_*+forwarded_from_* 引用；两窗口前均 TRUNCATE messages 从空表起始。

| 配置 | 全局 WAL 字节/消息 | INSERT messages WAL 字节/消息 | INSERT messages WAL 记录/消息 |
|---|---|---|---|
| 索引存在（A） | 2,953 | 2,331 | 15 |
| 索引剔除（B） | 2,645 | 2,332 | 15 |
...
|---|---|---|---|---|---|---|---|---|
| 2E2DDA1559AE03AD | 2,000 | 267.2 | 2,000 | 7 | 467 | 30,198 | 4,663,860 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_e93a46ca99dc"."messages" (  …` |
| AD398CE96D5C30F8 | 2,000 | 171.6 | 2,000 | 0 | 0 | 8,091 | 705,363 | `WITH upsert_conversation AS (     INSERT INTO "perf_e93a46ca99dc"."conversations" (       …` |
| 4D103CE8ABD01E23 | 2,000 | 54.8 | 4,000 | 0 | 0 | 0 | 0 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM UNNEST($1) AS …` |
| 403F8B358778114D | 2,003 | 14.8 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 2,003 | 9.3 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| CF1D5941D5B56432 | 2,000 | 7.1 | 0 | 0 | 0 | 0 | 0 | `BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED` |
| 878A9E463E585A2C | 2,003 | 6.7 | 2,003 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 1CA7E40EFC47C423 | 2,000 | 1.9 | 0 | 0 | 0 | 0 | 0 | `COMMIT` |
| D28E47E4803A0167 | 2,003 | 1.6 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| 92049CC8AB443DC6 | 2,003 | 0.9 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
| C0D0E27048376284 | 2,003 | 0.7 | 0 | 0 | 0 | 0 | 0 | `UNLISTEN *` |
...
| 265A210A7402B089 | 1 | 0.2 | 29 | 0 | 0 | 4 | 246 | `SELECT relname, COALESCE(n_tup_ins, $2), COALESCE(n_tup_upd, $3), COALESCE(n_tup_del, $4),…` |
...
|---|---|---|---|---|
| 2E2DDA1559AE03AD | 2,000 | 4,663,860 | 30,198 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_e93a46ca99dc"."messages" (  …` |
| AD398CE96D5C30F8 | 2,000 | 705,363 | 8,091 | `WITH upsert_conversation AS (     INSERT INTO "perf_e93a46ca99dc"."conversations" (       …` |
...
| messages | 2,047 | 0 | 0 | 0 | 0 |
| command_idempotency_ledger | 0 | 0 | 0 | 0 | 0 |
| conversation_members | 0 | 2,047 | 0 | 2,047 | -3 |
...
| conversations | 0 | 2,047 | 0 | 2,047 | -20 |
...
| queryid | calls | exec(ms) | rows | blks_read | dirtied | wal_records | wal_bytes | sql |
|---|---|---|---|---|---|---|---|---|
| 2E2DDA1559AE03AD | 2,000 | 271.9 | 2,000 | 7 | 467 | 30,195 | 4,664,272 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_e93a46ca99dc"."messages" (  …` |
| AD398CE96D5C30F8 | 2,000 | 177.5 | 2,000 | 0 | 0 | 8,091 | 705,363 | `WITH upsert_conversation AS (     INSERT INTO "perf_e93a46ca99dc"."conversations" (       …` |
| 4D103CE8ABD01E23 | 2,000 | 57.6 | 4,000 | 0 | 0 | 0 | 0 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM UNNEST($1) AS …` |
| 403F8B358778114D | 2,003 | 15.8 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 2,003 | 9.6 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| CF1D5941D5B56432 | 2,000 | 9.1 | 0 | 0 | 0 | 0 | 0 | `BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED` |
| 878A9E463E585A2C | 2,003 | 7.0 | 2,003 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 1CA7E40EFC47C423 | 2,000 | 2.1 | 0 | 0 | 0 | 0 | 0 | `COMMIT` |
| D28E47E4803A0167 | 2,003 | 1.8 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| 92049CC8AB443DC6 | 2,003 | 1.0 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
...
| queryid | calls | wal_bytes | wal_records | sql |
|---|---|---|---|---|
| 2E2DDA1559AE03AD | 2,000 | 4,664,272 | 30,195 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_e93a46ca99dc"."messages" (  …` |
| AD398CE96D5C30F8 | 2,000 | 705,363 | 8,091 | `WITH upsert_conversation AS (     INSERT INTO "perf_e93a46ca99dc"."conversations" (       …` |
| 4D103CE8ABD01E23 | 2,000 | 0 | 0 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM UNNEST($1) AS …` |
| 403F8B358778114D | 2,003 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 2,003 | 0 | 0 | `RESET ALL` |
...
| relationship_projection_rebuild_state | 0 | 0 | 0 | 0 | 0 |
| device_sync_cursors | 0 | 0 | 0 | 0 | 0 |
| messages | 1,945 | 0 | 0 | 0 | 0 |
| command_idempotency_ledger | 0 | 0 | 0 | 0 | 0 |
| conversation_members | 0 | 1,945 | 0 | 1,945 | -5 |
| account_cleanup_jobs | 0 | 0 | 0 | 0 | 0 |
| user_deletion_tombstones | 0 | 0 | 0 | 0 | 0 |
| relationship_projection_items | 0 | 0 | 0 | 0 | 0 |
| friendships | 0 | 0 | 0 | 0 | 0 |
| conversations | 0 | 1,945 | 0 | 1,945 | -5 |
| relationship_projection_history | 0 | 0 | 0 | 0 | 0 |
| outbox_replay_audit | 0 | 0 | 0 | 0 | 0 |
| conversation_membership_periods | 0 | 0 | 0 | 0 | 0 |
| relationship_sync_cursors | 0 | 0 | 0 | 0 | 0 |
