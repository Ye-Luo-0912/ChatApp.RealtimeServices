...
> 生成时间：2026-08-13 20:57:12 UTC；语料固定、随机种子 20260813；每配置 inbound 2000 条消息、排水 2000 条。
...
| C5FA397E315F9D09 | 40 | 37.4 | 2,000 | 0 | 46 | 5,642 | 0 | 745,422 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_9d52769e07ef"."o…` |
| E8836F9DF6B2FBE6 | 40 | 34.0 | 2,000 | 0 | 26 | 10,829 | 0 | 1,195,272 | `UPDATE "perf_9d52769e07ef"."outbox" AS item SET published_at_ms = $1, status = $4, locked_…` |
...
|---|---|---|---|---|---|
| E8836F9DF6B2FBE6 | 40 | 1,195,272 | 10,829 | 0 | `UPDATE "perf_9d52769e07ef"."outbox" AS item SET published_at_ms = $1, status = $4, locked_…` |
| C5FA397E315F9D09 | 40 | 745,422 | 5,642 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_9d52769e07ef"."o…` |
...
| 377846262928D7B3 | 2 | 4.1 | 2,000 | 0 | 0 | 2,042 | 0 | 110,960 | `DELETE FROM "perf_9d52769e07ef"."outbox" WHERE ctid IN (     SELECT ctid     FROM "perf_9d…` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| 377846262928D7B3 | 2 | 110,960 | 2,042 | 0 | `DELETE FROM "perf_9d52769e07ef"."outbox" WHERE ctid IN (     SELECT ctid     FROM "perf_9d…` |
| E11B52EF572B8535 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| B71A600DFCB91748 | 1 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
...
|---|---|---|---|---|---|---|---|---|---|
| E11B52EF572B8535 | 1 | 201.3 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| C5FA397E315F9D09 | 40 | 44.5 | 2,000 | 0 | 43 | 5,312 | 0 | 718,799 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_9d52769e07ef"."o…` |
| 26CAC6B03022E745 | 40 | 27.2 | 2,000 | 0 | 0 | 2,304 | 0 | 132,088 | `DELETE FROM "perf_9d52769e07ef"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| 403F8B358778114D | 83 | 0.6 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 40 | 718,799 | 5,312 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_9d52769e07ef"."o…` |
| 26CAC6B03022E745 | 40 | 132,088 | 2,304 | 0 | `DELETE FROM "perf_9d52769e07ef"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| E11B52EF572B8535 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| 403F8B358778114D | 83 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| B71A600DFCB91748 | 1 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 4040643821AD66AF | 83 | 0 | 0 | 0 | `RESET ALL` |
