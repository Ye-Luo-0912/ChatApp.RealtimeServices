...
> 生成时间：2026-08-13 23:05:45 UTC；语料固定、随机种子 20260813；每配置 inbound 2000 条消息。

| 配置 | 热路径 SQL/消息 | SQL 往返/消息 | 总执行时间/消息(ms) | WAL 字节/消息 | WAL 记录/消息 |
|---|---|---|---|---|---|
| A：合并（admission CTE + bundle，2 条数据语句） | 2 | 11.0 | 0.47 | 3,424 | 26.1 |
| B：super-bundle（合并为单条 CTE） | 1 | 8.0 | 0.49 | 2,772 | 21.0 |
...
|---|---|---|---|---|---|
| 2,000 | 1.0 | 0.20 | 19.1 | 2,779 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_e1c9eda4d6…` |
| 2,000 | 1.0 | 0.14 | 4.0 | 353 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     F…` |
...
|---|---|---|---|---|---|---|---|---|---|
| 2E2DDA1559AE03AD | 2,000 | 390.8 | 2,000 | 0 | 603 | 38,230 | 0 | 5,559,256 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_e1c9eda4d62f"."messages" (  …` |
| 9ADAC1E78D13A1E2 | 2,000 | 284.4 | 2,000 | 0 | 0 | 8,091 | 0 | 706,943 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM (VALUES ($2), …` |
| E11B52EF572B8535 | 1 | 201.3 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| 403F8B358778114D | 2,003 | 17.8 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| CF1D5941D5B56432 | 2,000 | 11.6 | 0 | 0 | 0 | 0 | 0 | 0 | `BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED` |
| 4040643821AD66AF | 2,003 | 10.2 | 0 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| 878A9E463E585A2C | 2,003 | 8.4 | 2,003 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 1CA7E40EFC47C423 | 2,000 | 2.4 | 0 | 0 | 0 | 0 | 0 | 0 | `COMMIT` |
| D28E47E4803A0167 | 2,003 | 2.3 | 0 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| B71A600DFCB91748 | 1 | 1.2 | 235 | 0 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 92049CC8AB443DC6 | 2,003 | 1.1 | 0 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
| C0D0E27048376284 | 2,003 | 0.9 | 0 | 0 | 0 | 0 | 0 | 0 | `UNLISTEN *` |
...
|---|---|---|---|---|---|
| 2E2DDA1559AE03AD | 2,000 | 5,559,256 | 38,230 | 0 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_e1c9eda4d62f"."messages" (  …` |
...
| 调用 | 调用/消息 | exec/消息(ms) | wal_records/消息 | wal_bytes/调用 | sql 片段 |
|---|---|---|---|---|---|
| 2,000 | 1.0 | 0.37 | 24.2 | 3,299 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     F…` |
| 2,003 | 1.0 | 0.01 | 0.0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 2,003 | 1.0 | 0.01 | 0.0 | 0 | `RESET ALL` |
| 2,003 | 1.0 | 0.00 | 0.0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 2,003 | 1.0 | 0.00 | 0.0 | 0 | `DISCARD SEQUENCES` |
...
| queryid | calls | exec(ms) | rows | blks_read | dirtied | wal_records | wal_fpi | wal_bytes | sql |
|---|---|---|---|---|---|---|---|---|---|
| F441132799D72C49 | 2,000 | 741.7 | 2,000 | 0 | 628 | 48,397 | 0 | 6,598,324 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM (VALUES ($2), …` |
| E11B52EF572B8535 | 1 | 200.3 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| 403F8B358778114D | 2,003 | 19.7 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 2,003 | 10.7 | 0 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| 878A9E463E585A2C | 2,003 | 9.8 | 2,003 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| D28E47E4803A0167 | 2,003 | 2.4 | 0 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| 92049CC8AB443DC6 | 2,003 | 1.2 | 0 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
| C0D0E27048376284 | 2,003 | 1.2 | 0 | 0 | 0 | 0 | 0 | 0 | `UNLISTEN *` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| F441132799D72C49 | 2,000 | 6,598,324 | 48,397 | 0 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM (VALUES ($2), …` |
| E11B52EF572B8535 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| 403F8B358778114D | 2,003 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 2,003 | 0 | 0 | 0 | `RESET ALL` |
| 878A9E463E585A2C | 2,003 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
