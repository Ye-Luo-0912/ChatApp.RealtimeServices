...
> 生成时间：2026-08-13 18:58:03 UTC；语料固定、随机种子 20260813；inbound 窗口 2000 条消息。
...
|---|---|---|---|---|---|---|---|---|
| 2E2DDA1559AE03AD | 2,000 | 319.7 | 2,000 | 0 | 449 | 30,192 | 4,656,894 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_2893d1048510"."messages" (  …` |
| AD398CE96D5C30F8 | 2,000 | 196.2 | 2,000 | 0 | 0 | 8,091 | 705,363 | `WITH upsert_conversation AS (     INSERT INTO "perf_2893d1048510"."conversations" (       …` |
| 4D103CE8ABD01E23 | 2,000 | 68.4 | 4,000 | 0 | 0 | 0 | 0 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM UNNEST($1) AS …` |
| 403F8B358778114D | 2,003 | 18.0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 2,003 | 11.3 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| 878A9E463E585A2C | 2,003 | 8.7 | 2,003 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| CF1D5941D5B56432 | 2,000 | 8.6 | 0 | 0 | 0 | 0 | 0 | `BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED` |
| 1CA7E40EFC47C423 | 2,000 | 2.6 | 0 | 0 | 0 | 0 | 0 | `COMMIT` |
| D28E47E4803A0167 | 2,003 | 2.1 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| 92049CC8AB443DC6 | 2,003 | 1.1 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
| C0D0E27048376284 | 2,003 | 0.9 | 0 | 0 | 0 | 0 | 0 | `UNLISTEN *` |
| B71A600DFCB91748 | 1 | 0.4 | 229 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 516E8F1759460B47 | 2,003 | 0.3 | 0 | 0 | 0 | 0 | 0 | `DISCARD TEMP` |
| 265A210A7402B089 | 1 | 0.3 | 29 | 0 | 0 | 3 | 181 | `SELECT relname, COALESCE(n_tup_ins, $2), COALESCE(n_tup_upd, $3), COALESCE(n_tup_del, $4),…` |
...
|---|---|---|---|---|
| 2E2DDA1559AE03AD | 2,000 | 4,656,894 | 30,192 | `WITH inserted_message AS MATERIALIZED (     INSERT INTO "perf_2893d1048510"."messages" (  …` |
| AD398CE96D5C30F8 | 2,000 | 705,363 | 8,091 | `WITH upsert_conversation AS (     INSERT INTO "perf_2893d1048510"."conversations" (       …` |
...
| messages | 1,961 | 0 | 0 | 0 | 0 |
| command_idempotency_ledger | 0 | 0 | 0 | 0 | 0 |
| conversation_members | 0 | 1,961 | 0 | 1,961 | 11 |
...
| conversations | 0 | 1,961 | 0 | 1,961 | 11 |
...
| queryid | calls | exec(ms) | rows | blks_read | dirtied | wal_records | wal_bytes | sql |
|---|---|---|---|---|---|---|---|---|
| C5FA397E315F9D09 | 40 | 63.4 | 2,000 | 0 | 173 | 11,399 | 1,860,778 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_2893d1048510"."o…` |
| E8836F9DF6B2FBE6 | 40 | 48.2 | 2,000 | 0 | 89 | 11,182 | 1,548,262 | `UPDATE "perf_2893d1048510"."outbox" AS item SET published_at_ms = $1, status = $4, locked_…` |
| 403F8B358778114D | 83 | 0.8 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| B71A600DFCB91748 | 1 | 0.8 | 232 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 4040643821AD66AF | 83 | 0.5 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| 878A9E463E585A2C | 83 | 0.4 | 83 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 265A210A7402B089 | 1 | 0.2 | 29 | 0 | 0 | 0 | 0 | `SELECT relname, COALESCE(n_tup_ins, $2), COALESCE(n_tup_upd, $3), COALESCE(n_tup_del, $4),…` |
| D28E47E4803A0167 | 83 | 0.1 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| 92049CC8AB443DC6 | 83 | 0.1 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
...
| queryid | calls | wal_bytes | wal_records | sql |
|---|---|---|---|---|
| C5FA397E315F9D09 | 40 | 1,860,778 | 11,399 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_2893d1048510"."o…` |
| E8836F9DF6B2FBE6 | 40 | 1,548,262 | 11,182 | `UPDATE "perf_2893d1048510"."outbox" AS item SET published_at_ms = $1, status = $4, locked_…` |
