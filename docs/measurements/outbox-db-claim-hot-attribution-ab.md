...
> 生成时间：2026-08-14 09:14:38 UTC；语料固定、随机种子 20260813；两配置均 fillfactor=50、每配置 inbound 2000 条消息；A 只 claim（不 delete）、B claim+delete（delete-on-complete）；两窗口前均 TRUNCATE outbox 从空表起始。
...
| C5FA397E315F9D09 | 10 | 22.7 | 2,000 | 0 | 358 | 5,675 | 0 | 763,341 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_e462f03f0358"."o…` |
| B71A600DFCB91748 | 1 | 0.5 | 234 | 0 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 265A210A7402B089 | 1 | 0.2 | 29 | 0 | 9 | 3 | 0 | 181 | `SELECT relname, COALESCE(n_tup_ins, $2), COALESCE(n_tup_upd, $3), COALESCE(n_tup_del, $4),…` |
...
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 10 | 763,341 | 5,675 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_e462f03f0358"."o…` |
...
## 结论（HOT residual root cause 归因）

**A（claim-only）与 B（claim+delete）的 claim HOT 命中率完全一致（同为 1,723/2,000 = 86%），
而两配置的 outbox dead tuple 差异巨大（A=450 vs B=2,286）。**

据此判定：

1. **delete 生成的 dead tuple 对 claim 的 HOT 命中率没有影响**。dead tuple 虽翻了 5 倍，
   但 claim 的 hot_updates 数逐条相同（1,723），说明页内版本链空间并未因 delete 而成为 claim HOT 的瓶颈。
   原假设「delete dead tuple 挤压 claim 页内空间导致 HOT<100%」被证伪。

2. **residual（14% non-HOT）是 claim 语句自身的固有属性**，与排水完整路径无关：
   - claim 更新的列（`locked_by`/`claim_token`/`locked_until_ms`/`attempt_count`）全部非索引，
     理论上整批可 HOT；
   - 但 INSERT 以 fillfactor=50 写入后，页内已占用一半空间；claim 版本链需要在同一页内追加
     新 tuple。当单行因加入 lease 列（large payload）后体积超过该页剩余可分配空间时，
     PostgreSQL 无法 HOT，回退为 non-HOT（跨页重定位 + 主键索引更新）。
   - 该比例由「页填充率 vs 行更新后体积」的物理布局决定，与 is-delete 无关，因此 A/B 恒等。

3. **工程结论**：fillfactor=50 已把 aggregate HOT 顶到 86%，且该上限不因删除路径而退化。
   想要进一步逼近 100% 只能通过降低 fillfactor（为更大的版本链预留空间）或压缩行体积
   （缩小 payload 列），两者都会牺牲空间/其它成本，收益有限（HOT 86%→更高仅影响 claim 的
   WAL 记录数，不影响 claim 已实现的 WAL 字节/消息 371）。
...
|---|---|---|---|---|---|---|---|---|---|
| E11B52EF572B8535 | 1 | 201.3 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| C5FA397E315F9D09 | 10 | 20.5 | 2,000 | 0 | 358 | 5,425 | 0 | 742,674 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_e462f03f0358"."o…` |
| 26CAC6B03022E745 | 10 | 6.7 | 2,000 | 0 | 0 | 2,290 | 0 | 131,362 | `DELETE FROM "perf_e462f03f0358"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 10 | 742,674 | 5,425 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_e462f03f0358"."o…` |
| 26CAC6B03022E745 | 10 | 131,362 | 2,290 | 0 | `DELETE FROM "perf_e462f03f0358"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| E11B52EF572B8535 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| B71A600DFCB91748 | 1 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
