...
> 生成时间：2026-08-19 06:40:19 UTC；语料固定、随机种子 20260813；每配置 inbound 2000 条消息、单会话（发送者固定 10_000_000_001、接收者固定 10_000_000_002）。

| 配置 | 热路径 SQL/消息 | SQL 往返/消息 | 总执行时间/消息(ms) | WAL 字节/消息 | WAL 记录/消息 |
|---|---|---|---|---|---|
| A：每条消息读发送者+接收者 user_state | 1 | 8.0 | 0.28 | 2,967 | 23.3 |
| B：sender-known（只读接收者 user_state） | 1 | 8.0 | 0.29 | 1,874 | 14.1 |
...
> 本次实测 B 较 A 每消息执行耗时差仅 -0.010 ms（0.28 → 0.29 ms），
...
|---|---|---|---|---|---|
| 2,000 | 1.0 | 0.17 | 24.2 | 3,184 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     F…` |
...
|---|---|---|---|---|---|---|---|---|---|
| F441132799D72C49 | 2,000 | 334.9 | 2,000 | 0 | 593 | 48,362 | 0 | 6,369,482 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM (VALUES ($2), …` |
| E11B52EF572B8535 | 1 | 201.4 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| 403F8B358778114D | 2,003 | 7.2 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 878A9E463E585A2C | 2,003 | 4.1 | 2,003 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 4040643821AD66AF | 2,003 | 3.7 | 0 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| D28E47E4803A0167 | 2,003 | 0.8 | 0 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| 92049CC8AB443DC6 | 2,003 | 0.6 | 0 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
| C0D0E27048376284 | 2,003 | 0.4 | 0 | 0 | 0 | 0 | 0 | 0 | `UNLISTEN *` |
...
|---|---|---|---|---|---|
| F441132799D72C49 | 2,000 | 6,369,482 | 48,362 | 0 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM (VALUES ($2), …` |
...
| 调用 | 调用/消息 | exec/消息(ms) | wal_records/消息 | wal_bytes/调用 | sql 片段 |
|---|---|---|---|---|---|
| 2,000 | 1.0 | 0.18 | 24.2 | 3,308 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     F…` |
| 2,003 | 1.0 | 0.00 | 0.0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 2,003 | 1.0 | 0.00 | 0.0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 2,003 | 1.0 | 0.00 | 0.0 | 0 | `RESET ALL` |
| 2,003 | 1.0 | 0.00 | 0.0 | 0 | `DISCARD SEQUENCES` |
...
| queryid | calls | exec(ms) | rows | blks_read | dirtied | wal_records | wal_fpi | wal_bytes | sql |
|---|---|---|---|---|---|---|---|---|---|
| 797F1A136F2748B2 | 2,000 | 354.6 | 2,000 | 0 | 628 | 48,422 | 0 | 6,617,520 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM (VALUES ($2), …` |
| E11B52EF572B8535 | 1 | 200.5 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| 403F8B358778114D | 2,003 | 8.1 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 878A9E463E585A2C | 2,003 | 4.4 | 2,003 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 4040643821AD66AF | 2,003 | 4.1 | 0 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| D28E47E4803A0167 | 2,003 | 0.8 | 0 | 0 | 0 | 0 | 0 | 0 | `DISCARD SEQUENCES` |
| 92049CC8AB443DC6 | 2,003 | 0.6 | 0 | 0 | 0 | 0 | 0 | 0 | `CLOSE ALL` |
| C0D0E27048376284 | 2,003 | 0.5 | 0 | 0 | 0 | 0 | 0 | 0 | `UNLISTEN *` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| 797F1A136F2748B2 | 2,000 | 6,617,520 | 48,422 | 0 | `WITH ordered_users AS MATERIALIZED (     SELECT DISTINCT t.user_id     FROM (VALUES ($2), …` |
| E11B52EF572B8535 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| 403F8B358778114D | 2,003 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 878A9E463E585A2C | 2,003 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
| 4040643821AD66AF | 2,003 | 0 | 0 | 0 | `RESET ALL` |
