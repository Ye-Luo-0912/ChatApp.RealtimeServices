...
> 生成时间：2026-09-03 22:50:28 UTC；语料固定、随机种子 20260813；每配置 inbound 2000 条消息、delete-on-complete 排水 2000 条；两窗口前均 TRUNCATE outbox 从空表起始。

| 配置 | claim WAL/消息 | claim WAL 记录/消息 | complete(删除) WAL/消息 | 排水合计 WAL/消息 | outbox HOT 命中率 |
|---|---|---|---|---|---|
| fillfactor=75（A） | 1,572 | 5.6 | 62 | 1,634 | 30% |
| fillfactor=50（B） | 1,020 | 2.7 | 66 | 1,086 | 87% |
...
| 404F44FB7C91EEBC | 40 | 40.0 | 2,000 | 0 | 177 | 11,226 | 221 | 3,145,016 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_fca03a7d245e"."o…` |
| C49C71344BD7CF03 | 1 | 26.3 | 3,211 | 0 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| B53912EFCBA6B266 | 40 | 5.1 | 2,000 | 0 | 0 | 2,200 | 0 | 124,400 | `DELETE FROM "perf_fca03a7d245e"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
...
|---|---|---|---|---|---|
| 404F44FB7C91EEBC | 40 | 3,145,016 | 11,226 | 221 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_fca03a7d245e"."o…` |
| B53912EFCBA6B266 | 40 | 124,400 | 2,200 | 0 | `DELETE FROM "perf_fca03a7d245e"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
...
| queryid | calls | exec(ms) | rows | blks_read | dirtied | wal_records | wal_fpi | wal_bytes | sql |
|---|---|---|---|---|---|---|---|---|---|
| 1F1A5B390EB81DF3 | 1 | 210.9 | 1 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| C49C71344BD7CF03 | 1 | 37.0 | 3,211 | 0 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 404F44FB7C91EEBC | 40 | 27.4 | 2,000 | 0 | 351 | 5,312 | 307 | 2,041,669 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_fca03a7d245e"."o…` |
| B53912EFCBA6B266 | 40 | 4.2 | 2,000 | 0 | 0 | 2,304 | 0 | 132,288 | `DELETE FROM "perf_fca03a7d245e"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| 7FD7FA9679F7127A | 1 | 1.4 | 29 | 0 | 1 | 0 | 0 | 0 | `SELECT relname, COALESCE(n_tup_ins, $2), COALESCE(n_tup_upd, $3), COALESCE(n_tup_del, $4),…` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| 404F44FB7C91EEBC | 40 | 2,041,669 | 5,312 | 307 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_fca03a7d245e"."o…` |
| B53912EFCBA6B266 | 40 | 132,288 | 2,304 | 0 | `DELETE FROM "perf_fca03a7d245e"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| 1F1A5B390EB81DF3 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| C49C71344BD7CF03 | 1 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 7FD7FA9679F7127A | 1 | 0 | 0 | 0 | `SELECT relname, COALESCE(n_tup_ins, $2), COALESCE(n_tup_upd, $3), COALESCE(n_tup_del, $4),…` |
| 9DFE0023C220BBE7 | 83 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
