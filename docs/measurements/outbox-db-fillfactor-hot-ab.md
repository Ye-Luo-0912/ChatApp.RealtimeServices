...
> 生成时间：2026-08-13 20:55:34 UTC；语料固定、随机种子 20260813；每配置 inbound 2000 条消息、delete-on-complete 排水 2000 条；两窗口前均 TRUNCATE outbox 从空表起始。
...
| C5FA397E315F9D09 | 40 | 54.1 | 2,000 | 0 | 387 | 11,263 | 0 | 1,834,676 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_31219b4ea950"."o…` |
| 26CAC6B03022E745 | 40 | 22.6 | 2,000 | 0 | 0 | 2,200 | 0 | 124,200 | `DELETE FROM "perf_31219b4ea950"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
...
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 40 | 1,834,676 | 11,263 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_31219b4ea950"."o…` |
| 26CAC6B03022E745 | 40 | 124,200 | 2,200 | 0 | `DELETE FROM "perf_31219b4ea950"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
...
| C5FA397E315F9D09 | 40 | 40.9 | 2,000 | 0 | 354 | 5,313 | 0 | 718,853 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_31219b4ea950"."o…` |
| 26CAC6B03022E745 | 40 | 21.9 | 2,000 | 0 | 0 | 2,304 | 0 | 132,088 | `DELETE FROM "perf_31219b4ea950"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| B71A600DFCB91748 | 1 | 0.8 | 238 | 0 | 0 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 403F8B358778114D | 83 | 0.6 | 0 | 0 | 0 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 83 | 0.4 | 0 | 0 | 0 | 0 | 0 | 0 | `RESET ALL` |
| 878A9E463E585A2C | 83 | 0.3 | 83 | 0 | 0 | 0 | 0 | 0 | `SELECT pg_advisory_unlock_all()` |
...
| queryid | calls | wal_bytes | wal_records | wal_fpi | sql |
|---|---|---|---|---|---|
| C5FA397E315F9D09 | 40 | 718,853 | 5,313 | 0 | `WITH candidates AS MATERIALIZED (     SELECT item.event_id     FROM "perf_31219b4ea950"."o…` |
| 26CAC6B03022E745 | 40 | 132,088 | 2,304 | 0 | `DELETE FROM "perf_31219b4ea950"."outbox" AS item USING UNNEST($1, $2) AS arr(event_id, cla…` |
| E11B52EF572B8535 | 1 | 0 | 0 | 0 | `SELECT pg_stat_force_next_flush(), pg_sleep($1)` |
| B71A600DFCB91748 | 1 | 0 | 0 | 0 | `SELECT queryid, query, calls, total_exec_time, rows,        shared_blks_read, shared_blks_…` |
| 403F8B358778114D | 83 | 0 | 0 | 0 | `SET SESSION AUTHORIZATION DEFAULT` |
| 4040643821AD66AF | 83 | 0 | 0 | 0 | `RESET ALL` |
