...
- Realtime 侧附件扫描闭环 → **wired**（2026-08-11）：扫描结果消费从主题到后台 worker 全链路接通。`Nats:Subjects:AttachmentScan`（`chat.attachment-scan`）经 `RealtimeQueueTopics` 映射到 `NatsAttachmentScanConsumer`（`IAttachmentScanConsumer`，fire-and-forget，反序列化失败/空负载丢弃）；`AttachmentScanCommand` 已注册 `RealtimeJsonSerializerContext`；`AttachmentScanWorker` 消费命令并调用 `IAttachmentScanProcessor.ProcessAsync` 驱动 Uploaded → Scanning → Available | Rejected（全程 state_version 条件更新，旧结果/重复命令不覆盖新状态），接入 `RealtimeNatsRegistration`（consumer）与 `RealtimeServicesRegistration`（processor + `AddHostedService`）。单命令异常/失败不阻断消费循环，共享并发门 `Mutation` 池过载时跳过命令（命令可重放）。新增 `AttachmentScanWorkerTests` 4 例。
