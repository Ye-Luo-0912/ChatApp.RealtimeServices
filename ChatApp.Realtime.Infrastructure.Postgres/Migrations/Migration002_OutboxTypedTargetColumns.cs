...
/// <summary>
/// Outbox 增加 typed <c>target_user_id</c> / <c>event_type</c>，并从 payload JSON 回填，
/// 以便用户清理按精确列匹配，而不再依赖 JSON LIKE。
/// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var commands = new[]
        {
            ...
            $"""
             ...
             """,
            // 无法从 payload 解析的历史脏行：落到 0，保证后续 NOT NULL；userId>0 的清理不会误删。
            $"""
             UPDATE {outbox}
             SET
