...
/// <para>
/// 每一次关系变更（Send / Accept / Decline / Remove / Block / Unblock）都在业务事务内
/// 写入一条日志，携带全局单调递增的 <see cref="ChangeSequence"/>。增量同步按
/// <c>change_sequence &gt; afterSequence</c> 读取，从而可靠表达删除、状态变更与同毫秒多次变更。
/// </para>
/// <para>
/// <see cref="Operation"/> = <see cref="RelationshipChangeOperation.Upsert"/> 时，
/// 客户端按 <see cref="ResourceId"/> upsert 本地条目（payload 反映最新状态）；
/// <see cref="RelationshipChangeOperation.Delete"/> 时，客户端按 <see cref="ResourceId"/>
/// 移除本地条目（tombstone）。
/// </para>
/// </summary>
